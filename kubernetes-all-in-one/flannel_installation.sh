...
set /coreos.com/network/config  '{ "Network": "172.20.0.0/16", "Backend": {"Type": "vxlan"}}'>&/dev/null

# Check the subnet configuration.
etcdctl \
--ca-file=${etcd_ca}/ca.pem --cert-file=${etcd_ca}/server.pem \
--key-file=${etcd_ca}/server-key.pem --endpoints="https://${etcd['etcd-master']}:2379,https://${etcd['etcd-01']}:2379,https://${etcd['etcd-02']}:2379" \
get /coreos.com/network/config

for node_ip in ${hosts[@]};
 do 
   if [ "${node_ip}" != "${hosts[gysl-master]}" ] ; then
     ssh root@${node_ip} "rm -f ${bin}/{flanneld,mk-docker-opts.sh}"
     ...
     ssh root@${node_ip} "[ -f /run/flanneld/subnet.env ] && rm -f /run/flanneld/subnet.env"
     ssh root@${node_ip} "sed -i '/EnvironmentFile/d' /usr/lib/systemd/system/docker.service"
     ssh root@${node_ip} "sed -i.bak_$(date +%d%H%M) '/ExecStart/i EnvironmentFile=\/run\/flanneld\/subnet.env' /usr/lib/systemd/system/docker.service"
